...
const dbGetRemunerationById = (record_id) => {
  return db
    .prepare(`SELECT * FROM remuneration_records WHERE record_id = ?;`)
    .get(record_id);
};

const dbGetAllRemunerationRecords = (limit = 100, offset = 0) => {
  return db
    .prepare(`SELECT * FROM remuneration_records LIMIT ? OFFSET ?;`)
    .all(limit, offset);
};
...
const dbSearchRemunerationRecords = ({
  ...
  let sql = `
    ...
    JOIN employees ON remuneration_records.employee_id = employees.employee_id
    JOIN departments ON remuneration_records.department_id = departments.department_id
    JOIN titles ON remuneration_records.title_id = titles.title_id
    WHERE 1=1
  `;
...
const dbGetRemunerationWithFeedbackById = (record_id) => {
  return db
    .prepare(`SELECT * FROM remuneration_with_feedback WHERE record_id = ?;`)
    .get(record_id);
};

const dbGetAllRemunerationWithFeedback = (limit = 100, offset = 0) => {
  return db
    .prepare(`SELECT * FROM remuneration_with_feedback LIMIT ? OFFSET ?;`)
    .all(limit, offset);
};
...
const dbSearchRemunerationWithFeedback = ({
  ...
  let sql = `
    SELECT * 
    FROM remuneration_with_feedback 
    WHERE 1=1
  `;
